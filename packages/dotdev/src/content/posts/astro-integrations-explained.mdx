...
## A Deeper Look
Internally, Astro Integrations, which are powered by [Vite](https://vite.dev), are nothing but a small function which is exported from the package you install (or code locally). They can take in one or multiple parameters and return an Object of the `AstroIntegration` type:
