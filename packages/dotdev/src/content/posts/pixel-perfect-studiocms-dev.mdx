...
Welcome to **Pixel Perfect**, a series of blog posts where I dive into the design process behind websites I have created. No matter if it's a SaaS-level landing page or a personal portfolio, there's a story behind each of them, so let's dive into it!

Today, we're taking a look at [studiocms.dev](https://studiocms.dev), the home of StudioCMS, an Astro-native CMS of which I am a core maintainer and the team's lead designer. We recently revamped the CMS's dashboard to look more modern, and created a UI library as part of that ([about which I wrote a blogpost!](/blog/building-a-ui-lib)). It became apparent fairly quickly that we would not be able to keep the current homepage, as it had aged rather poorly and no longer reflected our brand design, along with other issues. I've created a [backup on the WayBackMachine](https://webcf.waybackmachine.org/web/20250118060937/https://studiocms.dev/), so you can look at the old version if you want to.
...
At the time of writing, we have not finished the video yet, so you be the judge of how well we did.
...
We first do what I like to call a "hard-cut" by providing a full-width element with a different background color. This is meant to be a subtle nod to the user, saying "You've reached the end of this type of content". Within that banner, we start transitioning from technical details to social proof by showing off download stats, the number of Discord members and GitHub stars we've gathered.

Next is the actual social proof section, where community members have written messages to show off their opinion on why StudioCMS is great. It's a common technique to show off other people using and liking your product, and it's what services like Trustpilot are built on. Essentially, they're a way to peer pressure users into using your product! If everyone else already uses it, why aren't you already?
